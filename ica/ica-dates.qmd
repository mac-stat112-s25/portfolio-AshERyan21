...
We should only use this in extreme because it can take out a lot of data that at various times we can use for other purposes. 
...
#Month pulls month, label puts it in the words.
```

```{r}
# What does this number mean?
week(today)
#Does this mean that this is the ninth week of the year? Yes
```

```{r}
# What do these lines produce / what's their difference?
mday(today) #This is the day of the month we are at. 
yday(today)  # This is often called the "Julian day" - this is the days total into the year
```

```{r}
# What do these lines produce / what's their difference?
wday(today) #This gives us the day of the week we are on - Starts with Sunday
wday(today, label = TRUE) #This gives us the day as letters. 
```

```{r}
# What do the results of these 2 lines tell us?
today >= ymd("2024-02-14") #We are past this date
today < ymd("2024-02-14") #We are not less than this date --> allows for comparisons with dates. 
```
